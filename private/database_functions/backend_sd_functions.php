...
function dir_add($req_vals)
{
    ...
    if(count($errors) == 0)
    {
        $insert_statement = $conn->prepare("INSERT INTO `site_directory` (p_id, slug, title, description, robots, public) VALUES (?, ?, ?, ?, ?, ?)");
 ...
 */
 function dir_update($req_vals)
 {
     ...
     if(count($errors) == 0)
     {
         $update_statement = $conn->prepare("UPDATE `site_directory` SET p_id=?, slug=?, title=?, description=?, robots=?, public=? WHERE id=?");
         $update_statement->bind_param('issssii',
             $directory_pid,
             $directory_slug, 
             $directory_title, 
             $directory_description, 
